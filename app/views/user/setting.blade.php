...
<form action="<?=URL?>chu-de/luu-thiet-lap/<?=$_GET['id_quiz']?>/subject/<?=$_GET['id_subject']?>" method="post" class="row g-3 needs-validation">
  <div class="mb-3 col-lg-6">
    ...
    <input type="datetime-local" name="start_time"  class="form-control" value="<?=$data['model'] ->start_time?>">
    <br>
    <input type="text" name="start_default"  value="<?=$data['model'] ->start_time?>">
  </div>
  <div class="mb-3  col-lg-6">
    <label for="exampleInputPassword1" class="form-label">Thời Gian kết thúc</label>
    <input type="datetime-local" name="end_time"  class="form-control" value="<?=$data['model'] ->end_time?>">
    <br>
    <input type="text" name="end_default"  value="<?=$data['model'] ->end_time?>">
  </div>
  <div class="mb-3 col-lg-6">
    <label for="exampleInputPassword1" class="form-label">Thời gian làm bài</label>
    <input type="text" name="minutes"  class="form-control" value="<?=$data['model'] ->minutes ?>">
